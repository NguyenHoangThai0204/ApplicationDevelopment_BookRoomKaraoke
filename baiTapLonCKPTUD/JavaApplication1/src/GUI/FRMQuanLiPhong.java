...
//        ));
    ...
    public void loadDanhSachPhong() {
		...
		for (Phong p : lsP) {
			...
			modelPhong.addRow(new Object[] {p.getMaPhong(), loaiP.getTenLoaiPhong(), dfGiaP.format(p.getGiaPhong()), p.getTinhTrangPhong(),p.getSucChua(),p.getMoTa(),p.getSucChua(),p.getMoTa() });
		}
	}
	...
	public void sortLoaiPhong(Phong p) {
		clearTable();
	...
	public void themPhong() {
		if(regex.regexGiaP(txtGiaPhong)) {
			...
			LoaiPhong loaiP = new LoaiPhong(daoLoaiP.getMaLoaiPTheoTen(jcboChucVu.getSelectedItem().toString()));
	...
	private void loadDanhSachPhongTheoLoai(ArrayList<Phong> p2) {
		clearTable();
		String maLoai = daoLoaiP.getMaLoaiPTheoTen(jTextField1.getText());
	...
	public void loadDanhSachPhongTheoMa(ArrayList<Phong> lstP) {
		clearTable();
		ArrayList<Phong> lsP = daoPhong.getPhongTheoMaP(jTextField1.getText());
		for (Phong p : lsP) {
			LoaiPhong loaiP = daoLoaiP.getLoaiPhongTheoMa(p.getLoaiPhong().getMaLoaiPhong());
			modelPhong.addRow(new Object[] {p.getMaPhong(), loaiP.getTenLoaiPhong(), dfGiaP.format(p.getGiaPhong()), p.getTinhTrangPhong(), p.getTinhTrangPhong(),p.getSucChua(),p.getMoTa() });
		}
	...
	public void loadDanhSachPhongTheoTinhTrang(ArrayList<Phong> lstP) {
		clearTable();
		ArrayList<Phong> lsP = daoPhong.getPhongTheoTinhTrang(jTextField1.getText());
		for (Phong p : lsP) {
			LoaiPhong loaiP = daoLoaiP.getLoaiPhongTheoMa(p.getLoaiPhong().getMaLoaiPhong());
			modelPhong.addRow(new Object[] {p.getMaPhong(), loaiP.getTenLoaiPhong(), dfGiaP.format(p.getGiaPhong()), p.getTinhTrangPhong(), p.getTinhTrangPhong(),p.getSucChua(),p.getMoTa() });
		}
			...
			if (!jTextField1.getText().equals("") && !jTextField1.getText().equals("Tìm theo mã phòng, loại phòng, tình trạng phòng")) {
				if(regex.regexTimPhong(jTextField1)) {
					if (jTextField1.getText().trim().matches(regexMaPhong)) {
						lstP = daoPhong.getPhongTheoMaP(jTextField1.getText().trim());
						loadDanhSachPhongTheoMa(lstP);
					}
				if(regex.regexTimKiemLoaiPhong(jTextField1)) {
						lstP = daoPhong.getPhongTheoLoai(daoLoaiP.getMaLoaiPTheoTen(jTextField1.getText()).trim());
						loadDanhSachPhongTheoLoai(lstP);
				}
				if(regex.regexTimKiemTinhTrang(jTextField1)) {
						lstP = daoPhong.getPhongTheoTinhTrang(jTextField1.getText());
